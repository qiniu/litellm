...
class ContextCachingEndpoints(VertexBase):
    ...
    def check_cache(
        ...
        try:
            ...
            verbose_proxy_logger.debug(
                f"🔍 Requesting Google API cache lookup - cache_key={cache_key[:50]}..., "
            ...
            verbose_proxy_logger.debug(
                f"✅ Google API cache lookup succeeded - cache_key={cache_key[:50]}..., "
                f"status_code={resp.status_code}, PID={os.getpid()}"
            )
        except httpx.HTTPStatusError as e:
            verbose_proxy_logger.error(
                f"❌ Google API cache lookup failed - cache_key={cache_key[:50]}..., "
            ...
            verbose_proxy_logger.error(
                f"❌ Google API cache lookup exception - cache_key={cache_key[:50]}..., "
                ...
                if cache_id:
                    ...
                    if expire_time:
                        ...
                        if remaining_ttl is None:
                            ...
                            verbose_proxy_logger.debug(
                                f"⚠️  Google API found cache but it is expired - "
                        ...
                        verbose_proxy_logger.debug(
                            f"✅ Google API found cache - cache_id={cache_id}, "
                            f"name={display_name[:30]}..., expire_time={expire_time}, "
                            f"remaining_ttl={ttl_seconds:.1f}s, PID={os.getpid()}"
                        )
                    else:
                        # If no expireTime, use default TTL
                        ttl_seconds = 3600.0
                        verbose_proxy_logger.debug(
                            f"✅ Google API found cache (no expireTime) - cache_id={cache_id}, "
                            f"name={display_name[:30]}..., using default_ttl={ttl_seconds}s, PID={os.getpid()}"
                        )

                    # Store in local cache with accurate TTL
                    verbose_proxy_logger.debug(
                        f"Vertex AI context cache: storing cache entry locally - "
                        f"cache_id={cache_id}, cache_key={cache_key[:50]}..., ttl={ttl_seconds:.1f}s"
                    )
        ...
        verbose_proxy_logger.debug(
            f"❌ Google API did not find matching cache - cache_key={cache_key[:50]}..., "
            f"total_cached_items={len(all_cached_items.get('cachedContents', []))}, PID={os.getpid()}"
        )
        return None
    ...
    async def async_check_cache(
        ...
        try:
            ...
            verbose_proxy_logger.debug(
                f"🔍 [async]Requesting Google API cache lookup - cache_key={cache_key[:50]}..., "
                f"project={vertex_project}, location={vertex_location}, "
                f"provider={custom_llm_provider}, PID={os.getpid()}, url={url}"
            )
            ...
            verbose_proxy_logger.debug(
                f"✅ [async]Google API cache lookup succeeded - cache_key={cache_key[:50]}..., "
                f"status_code={resp.status_code}, PID={os.getpid()}"
            )
        except httpx.HTTPStatusError as e:
            verbose_proxy_logger.error(
                f"❌ [async]Google API cache lookup failed - cache_key={cache_key[:50]}..., "
                f"status_code={e.response.status_code}, PID={os.getpid()}"
            )
            if e.response.status_code == 403:
                return None
            ...
            verbose_proxy_logger.error(
                f"❌ [async]Google API cache lookup exception - cache_key={cache_key[:50]}..., "
                f"error={str(e)}, PID={os.getpid()}"
            )
            raise VertexAIError(status_code=500, message=str(e))
        raw_response = resp.json()
                ...
                if cache_id:
                    ...
                    if expire_time:
                        ...
                        if remaining_ttl is None:
                            # Already expired, don't store in local cache
                            verbose_proxy_logger.debug(
                                f"⚠️  [async]Google API found cache but it is expired - "
                                f"cache_id={cache_id}, name={display_name[:30]}..., "
                                f"expire_time={expire_time}, PID={os.getpid()}"
                            )
                            return None
                        ttl_seconds = remaining_ttl
                        verbose_proxy_logger.debug(
                            f"✅ [async]Google API found cache - cache_id={cache_id}, "
                            f"name={display_name[:30]}..., expire_time={expire_time}, "
                            f"remaining_ttl={ttl_seconds:.1f}s, PID={os.getpid()}"
                        )
                    else:
                        ...
                        ttl_seconds = 3600.0
                        verbose_proxy_logger.debug(
                            f"✅ [async]Google API found cache (no expireTime) - cache_id={cache_id}, "
                            f"name={display_name[:30]}..., using default_ttl={ttl_seconds}s, PID={os.getpid()}"
                        )

                    # Store in local cache with accurate TTL
                    verbose_proxy_logger.debug(
                        f"Vertex AI context cache: [async] storing cache entry locally - "
                        f"cache_id={cache_id}, cache_key={cache_key[:50]}..., ttl={ttl_seconds:.1f}s"
                    )
                    self.local_cache_manager.set_cache(
                        cache_key=cache_key,
        ...
        verbose_proxy_logger.error(
            f"❌ [async]Google API did not find matching cache - cache_key={cache_key[:50]}..., "
            f"total_cached_items={len(all_cached_items.get('cachedContents', []))}, PID={os.getpid()}"
        )
        return None
    ...
    def check_and_create_cache(
        ...
        """
        verbose_proxy_logger.debug(
            f"🚀 [TEST] check_and_create_cache called - model={model}, "
            f"provider={custom_llm_provider}, messages_count={len(messages)}"
        )
        if cached_content is not None:
            verbose_proxy_logger.debug(
                f"Vertex AI context cache: using provided cached_content={cached_content}, "
                f"skip cache handling"
            )
        ...
        verbose_proxy_logger.debug(
            f"Vertex AI context cache: message split complete - "
            f"cached_messages={len(cached_messages)}, non_cached_messages={len(non_cached_messages)}"
        )

        if len(cached_messages) == 0:
            verbose_proxy_logger.debug(
                "Vertex AI context cache: no cache-eligible messages, skip cache handling"
            )
        ...
        verbose_proxy_logger.debug(
            f"Vertex AI context cache: generated cache_key={generated_cache_key[:50]}... "
        ...
        if local_cache_id is not None:
            ...
            verbose_proxy_logger.debug(
                f"Vertex AI context cache: ✅ local cache hit - cache_id={local_cache_id}, "
                f"cache_key={generated_cache_key[:50]}..."
            )
            return non_cached_messages, optional_params, local_cache_id

        verbose_proxy_logger.debug(
            f"Vertex AI context cache: ❌ local cache miss - cache_key={generated_cache_key[:50]}..., "
            f"querying Google API..."
        )

        ## CHECK IF CACHED ON GOOGLE (network call, but only if not in local cache)
        verbose_proxy_logger.debug(
            f"Vertex AI context cache: querying Google API cache cache_key={generated_cache_key[:50]}..."
        )
        ...
        if google_cache_name:
            verbose_proxy_logger.debug(
                f"Vertex AI context cache: ✅ Google API cache hit - cache_id={google_cache_name}, "
                f"cache_key={generated_cache_key[:50]}..."
            )
            return non_cached_messages, optional_params, google_cache_name

        verbose_proxy_logger.debug(
            f"Vertex AI context cache: ❌ Google API cache not found - cache_key={generated_cache_key[:50]}..., "
        ...
        verbose_proxy_logger.debug(
            f"🚀 Requesting Google API cache creation - cache_key={generated_cache_key[:50]}..., "
            f"project={vertex_project}, location={vertex_location}, "
            f"provider={custom_llm_provider}, PID={os.getpid()}, url={url}"
        )
        ...
        try:
            ...
            verbose_proxy_logger.debug(
                f"✅ Google API cache creation succeeded - cache_key={generated_cache_key[:50]}..., "
                f"status_code={response.status_code}, PID={os.getpid()}"
            )
        except httpx.HTTPStatusError as err:
            error_code = err.response.status_code
            verbose_proxy_logger.error(
                f"❌ Google API cache creation failed - cache_key={generated_cache_key[:50]}..., "
                f"status_code={error_code}, PID={os.getpid()}, "
                f"error={err.response.text[:200]}"
            )
            raise VertexAIError(status_code=error_code, message=err.response.text)
        except httpx.TimeoutException:
            verbose_proxy_logger.error(
                f"❌ Google API cache creation timeout - cache_key={generated_cache_key[:50]}..., "
                f"PID={os.getpid()}"
            )
        ...
        verbose_proxy_logger.debug(
            f"Vertex AI context cache: ✅ created new cache - cache_id={cache_id}, "
            f"cache_key={generated_cache_key[:50]}..., ttl={ttl_seconds}s, "
            f"storing to local cache..."
        )
    ...
    async def async_check_and_create_cache(
        ...
        """
        verbose_proxy_logger.debug(
            f"🚀 [async] async_check_and_create_cache called - model={model}, "
            f"provider={custom_llm_provider}, messages_count={len(messages)}"
        )
        if cached_content is not None:
            verbose_proxy_logger.debug(
                f"Vertex AI context cache: [async] using provided cached_content={cached_content}, "
                f"skip cache handling"
            )
            return messages, optional_params, cached_content
        ...
        verbose_proxy_logger.debug(
            f"Vertex AI context cache: [async] message split complete - "
            f"cached_messages={len(cached_messages)}, non_cached_messages={len(non_cached_messages)}"
        )

        if len(cached_messages) == 0:
            verbose_proxy_logger.debug(
                "Vertex AI context cache: [async] no cache-eligible messages, skip cache handling"
            )
            return messages, optional_params, None
        ...
        verbose_proxy_logger.debug(
            f"Vertex AI context cache: [async] generated cache_key={generated_cache_key[:50]}... "
            f"(project={vertex_project}, location={vertex_location}, provider={custom_llm_provider})"
        )
        ...
        if local_cache_id is not None:
            # Found valid cache locally, return immediately
            verbose_proxy_logger.debug(
                f"Vertex AI context cache: ✅ [async] local cache hit - cache_id={local_cache_id}, "
                f"cache_key={generated_cache_key[:50]}..."
            )
            return non_cached_messages, optional_params, local_cache_id

        verbose_proxy_logger.debug(
            f"Vertex AI context cache: ❌ [async] local cache miss - cache_key={generated_cache_key[:50]}..., "
            f"querying Google API..."
        )

        ## CHECK IF CACHED ON GOOGLE
        verbose_proxy_logger.debug(
            f"Vertex AI context cache: [async] querying Google API cache cache_key={generated_cache_key[:50]}..."
        )
        ...
        if google_cache_name:
            verbose_proxy_logger.debug(
                f"Vertex AI context cache: ✅ [async] Google API cache hit - cache_id={google_cache_name}, "
                f"cache_key={generated_cache_key[:50]}..."
            )
            return non_cached_messages, optional_params, google_cache_name

        verbose_proxy_logger.debug(
            f"Vertex AI context cache: ❌ [async] Google API cache not found - cache_key={generated_cache_key[:50]}..., "
            f"creating new cache..."
        )
        ...
        verbose_proxy_logger.debug(
            f"🚀 [async]Requesting Google API cache creation - cache_key={generated_cache_key[:50]}..., "
            f"project={vertex_project}, location={vertex_location}, "
            f"provider={custom_llm_provider}, PID={os.getpid()}, url={url}"
        )
        ...
        try:
            ...
            verbose_proxy_logger.debug(
                f"✅ [async]Google API cache creation succeeded - cache_key={generated_cache_key[:50]}..., "
                f"status_code={response.status_code}, PID={os.getpid()}"
            )
        except httpx.HTTPStatusError as err:
            error_code = err.response.status_code
            verbose_proxy_logger.error(
                f"❌ [async]Google API cache creation failed - cache_key={generated_cache_key[:50]}..., "
                f"status_code={error_code}, PID={os.getpid()}, "
                f"error={err.response.text[:200]}"
            )
            raise VertexAIError(status_code=error_code, message=err.response.text)
            ...
            verbose_proxy_logger.error(
                f"❌ [async]Google API cache creation timeout - cache_key={generated_cache_key[:50]}..., "
                f"PID={os.getpid()}"
            )
            raise VertexAIError(status_code=408, message="Timeout error occurred.")
        ...
        verbose_proxy_logger.debug(
            f"Vertex AI context cache: ✅ [async] created new cache - cache_id={cache_id}, "
            f"cache_key={generated_cache_key[:50]}..., ttl={ttl_seconds}s, "
            f"storing to local cache..."
        )
