"""
Optimized version of vertex_ai_context_caching.py with local cache manager.

Changes:
1. Added local cache manager to avoid redundant network requests
2. Modified check_and_create_cache to check local cache first
3. Store cache ID and TTL locally after creation
4. Extract TTL from cached messages to calculate expiry
"""
...
class ContextCachingEndpointsOptimized(VertexBase):
    """
    Optimized context caching endpoints with local cache manager.

    Reduces network overhead by caching cache IDs locally.
    """
    ...
    def check_cache(
        ...
        """
        Checks if content already cached - with local cache optimization.

        Now checks local cache first before making network request.
        ...
        if local_cache_id is not None:
            # Found in local cache and not expired
            return local_cache_id

        # Not in local cache or expired, check with Google API
        ...
        for cached_item in all_cached_items["cachedContents"]:
            ...
            if display_name is not None and display_name == cache_key:
                ...
                # OPTIMIZATION: Store in local cache if we found it
                # Extract TTL from expire_time if available
                expire_time = cached_item.get("expireTime")
                if expire_time and cache_id:
                    # Calculate remaining TTL
                    # For simplicity, we'll use a default TTL here
                    # In production, you'd parse expireTime and calculate remaining seconds
    ...
    async def async_check_cache(
        ...
        """
        Async version - checks if content already cached with local cache optimization.
    ...
    def check_and_create_cache(
        ...
        """
        Optimized version with local cache manager.

        Checks local cache first, then Google API, then creates new cache if needed.
        """
        ...
        # OPTIMIZATION 1: Check local cache first (no network call, with project/location scope)
        local_cache_id = self.local_cache_manager.get_cache(
        ...
        # OPTIMIZATION 2: Store newly created cache in local cache manager (with project/location scope)
    ...
    async def async_check_and_create_cache(
        ...
        """
        Async optimized version with local cache manager.
        """
        if cached_content is not None:
            return messages, optional_params, cached_content
        ...
        # OPTIMIZATION 1: Check local cache first (with project/location scope)
        local_cache_id = self.local_cache_manager.get_cache(
            cache_key=generated_cache_key,
            vertex_project=vertex_project,
            vertex_location=vertex_location,
        ...
        # OPTIMIZATION 2: Store in local cache (with project/location scope)
        ttl_str = cached_content_request_body.get("ttl")
        if ttl_str:
            ttl_seconds = parse_ttl_to_seconds(ttl_str)
        else:
