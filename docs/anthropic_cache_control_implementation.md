...
| 提供商 | 支持 cache_control | anthropic-beta header 支持 | 说明 |
|--------|-------------------|---------------------------|------|
| **Anthropic** | ✅ | ✅ 完整支持所有 beta headers | 原生 API,完整功能 |
| **Bedrock** | ✅ | ⚠️ **部分支持** | 支持大部分 beta features,但 **不支持** `prompt-caching-2024-07-31` |
| **Vertex AI** | ✅ | ❌ 不支持 | 所有 beta headers 都会被忽略 |

#### Bedrock anthropic-beta 支持详情

**AWS Bedrock 支持的 beta headers** (通过 `anthropic_beta` 参数传递):

```python
# ✅ Bedrock 支持的 beta features
supported_bedrock_betas = [
    "context-1m-2025-08-07",           # 1M context window
    "computer-use-2025-01-24",         # Computer use tools (新版本)
    "computer-use-2024-10-22",         # Computer use tools (旧版本)
    ...
    "dev-full-thinking-2025-05-14",    # Full thinking development
    "advanced-tool-use-2025-11-20",    # Tool search & programmatic calling
    "effort-2025-11-24",               # Effort parameter (Opus 4.5)
]

# ❌ Bedrock 不支持 (会被自动过滤)
unsupported_bedrock_betas = [
    "prompt-caching-2024-07-31",  # ← 通过 cache_control 字段识别,不需要 beta header
]
```

**Prompt Caching 在 Bedrock 上的特殊处理**:

```python
# Bedrock 识别 prompt caching 的方式:
# - 通过请求体中的 cache_control 字段识别
# - 不需要 "prompt-caching-2024-07-31" beta header
# - LiteLLM 会自动过滤掉这个 header

# ✅ 正确用法 (无需手动添加 beta header)
response = await litellm.acompletion(
            ...
            "content": [
                {
                    "type": "text",
                    "text": "Hello",
                    "cache_control": {"type": "ephemeral"}  # ← 自动生效
                }
            ]
        }
    ]
    # LiteLLM 不会添加 "prompt-caching-2024-07-31" header
)
```
...
```python
# Bedrock 使用 1M context window
response = await litellm.acompletion(
    model="bedrock/anthropic.claude-3-5-sonnet-20241022-v2:0",
    messages=[...],
    extra_headers={
        "anthropic-beta": "context-1m-2025-08-07"  # ← 会被转换为 anthropic_beta 参数
    }
)

# Bedrock 使用 computer use tools (自动添加 beta)
response = await litellm.acompletion(
    model="bedrock/anthropic.claude-3-5-sonnet-20241022-v2:0",
    messages=[...],
    tools=[
    ...
    # LiteLLM 自动添加: anthropic_beta = ["computer-use-2025-01-24"]
)
```

#### Vertex AI 特殊处理

**Vertex AI 完全不支持 anthropic-beta headers**:

```python
# Vertex AI - 所有 beta headers 都会被忽略
response = await litellm.acompletion(
            ...
            "content": [
                {
                    "type": "text",
                    "text": "Hello",
                    "cache_control": {"type": "ephemeral"}  # ← cache_control 仍然有效
                }
            ]
        }
    ],
    extra_headers={
        "anthropic-beta": "context-1m-2025-08-07"  # ← 会被忽略,不会导致错误
    }
)
```

**实现细节** (litellm/llms/anthropic/common_utils.py:295-301):

```python
def get_anthropic_headers(..., is_vertex_request: bool = False, ...):
    # ...构建 betas set...

    # Vertex AI: 完全忽略 beta headers
    if is_vertex_request is True:
        pass  # 不添加任何 anthropic-beta header
    elif len(betas) > 0:
...
**Messages API** (litellm/llms/bedrock/messages/invoke_transformations/anthropic_claude3_transformation.py):

```python
def transform_anthropic_messages_request(...):
    # 从 headers 中提取用户提供的 beta headers
    beta_set = set(get_anthropic_beta_from_headers(headers))

    # 自动检测需要的 beta headers
    auto_betas = anthropic_model_info.get_anthropic_beta_list(
        ...
        prompt_caching_set=False,  # ← 注意: 不为 prompt caching 添加 beta
        file_id_used=anthropic_model_info.is_file_id_used(messages),
        ...
    )
...
**Converse API** (litellm/llms/bedrock/chat/converse_transformation.py):
...
def _process_tools_and_beta(...):
    ...
    # Computer use tools 自动添加对应的 beta
    if computer_use_tools:
        anthropic_beta_list.append("computer-use-2024-10-22")

    # 添加到 additionalModelRequestFields
    if anthropic_beta_list:
        unique_betas = list(dict.fromkeys(anthropic_beta_list))  # 去重
...
### Q3: 可以在 Bedrock 上使用 prompt caching 吗?

**A**: 可以!但有一些区别:

```python
# ✅ Prompt Caching 在 Bedrock 上正常工作
response = await litellm.acompletion(
    model="bedrock/anthropic.claude-3-5-sonnet-20241022-v2:0",
    messages=[
        {
    ...
    # LiteLLM 会自动处理,不需要手动添加 "prompt-caching-2024-07-31" header
)

# ✅ Bedrock 支持其他 beta features
response = await litellm.acompletion(
    model="bedrock/anthropic.claude-3-5-sonnet-20241022-v2:0",
    messages=[...],
    extra_headers={
        "anthropic-beta": "context-1m-2025-08-07"  # ← 可以使用其他 beta features
    }
)

# ❌ 错误 - 不要手动添加 prompt-caching beta header
response = await litellm.acompletion(
    model="bedrock/anthropic.claude-3-5-sonnet-20241022-v2:0",
    messages=[...],
    extra_headers={
        "anthropic-beta": "prompt-caching-2024-07-31"  # ← 会被忽略(无效)
    }
)
```

**关键点**:
- Bedrock 通过 `cache_control` 字段识别 prompt caching
- **不需要** `prompt-caching-2024-07-31` beta header
- 支持其他 beta features (如 context-1m, computer-use 等)
